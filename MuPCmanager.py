...
#ビットマップウィンドーのシステムエクスクルーシブメッセージ
bit_window_top    = b"\xF0\x43\x10\x4C\x07\x00\x00"
bit_window_bottom = b"\xF7" 
bit_window_add    = b"\x00"
...
try:
  ...
  while True:
    ...
    #メッセージウィンドーを表示
    out.write_sys_ex(0,msg_window_top+strTohexArray(("CPU Usage:"+str(cpu)+"%").ljust(16," ")+("RAM Usage:"+str(mem.percent)+"%").ljust(16," "))+msg_window_bottom)

    # out.write_sys_ex(0,msg_window_top+strTohexArray(("RAM Free:"+str(int(psutil.virtual_memory().available/1024/1024))+"MB").ljust(16," ") +("RAM Usage:"+str(mem.percent)+"%").ljust(16," ")) +msg_window_bottom)


    # #ビットマップウィンドーを表示
    # out.write_sys_ex(0,bit_window_top +bit_window_add +
    # b"\x7F"+ b"\x7F" + b"\x7F"+ b"\x7F"+ b"\x7F"+ b"\x7F"+ b"\x7F" +b"\x7F"+ b"\x7F" + b"\x7F"+ b"\x7F"+ b"\x7F"+ b"\x7F"+ b"\x7F"+b"\x7F"+
    # bit_window_add +
    # b"\x7F"+ b"\x7F" + b"\x7F"+ b"\x7F"+ b"\x7F"+ b"\x7F"+ b"\x7F" +b"\x7F"+ b"\x7F" + b"\x7F"+ b"\x7F"+ b"\x7F"+ b"\x7F"+ b"\x7F"+b"\x7F"+
    # bit_window_add +
    # b"\x60"+ b"\x60" + b"\x60"+ b"\x60"+ b"\x60"+b"\x60"+ b"\x60"+ b"\x60"+ b"\x60"+ b"\x60"+ b"\x60"+ b"\x60"+ b"\x60"+ b"\x60"+ b"\x60"+ 
    # bit_window_bottom)
